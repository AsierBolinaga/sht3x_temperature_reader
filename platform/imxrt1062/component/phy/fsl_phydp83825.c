...
status_t PHY_DP83825_Init(phy_handle_t *handle, const phy_config_t *config)
{
    ...
    if (result == kStatus_Success)
    {
        ...
        result = PHY_DP83825_WRITE(handle, PHY_LEDCR_REG, 0x0280e);
        if (result != kStatus_Success)
        {
            return result;
        }
        result = PHY_DP83825_WRITE(handle, PHY_RCSR_REG, 0x81);
        if (result != kStatus_Success)
        {
            return result;
        }
...
status_t PHY_DP83825_GetLinkSpeedDuplex(phy_handle_t *handle, phy_speed_t *speed, phy_duplex_t *duplex)
{
    ...
    result = PHY_DP83825_READ(handle, PHY_CONTROL1_REG, &regValue);
    if (result == kStatus_Success)
    {
        if (speed != NULL)
        {
...
status_t PHY_DP83825_EnableLoopback(phy_handle_t *handle, phy_loop_t mode, phy_speed_t speed, bool enable)
{
    ...
    if (enable)
    {
        ...
        else
        {
            ...
            result = PHY_DP83825_READ(handle, PHY_CONTROL2_REG, &regValue);
            if (result == kStatus_Success)
            {
                return PHY_DP83825_WRITE(handle, PHY_CONTROL2_REG, (regValue | PHY_CTL2_REMOTELOOP_MASK));
            }
        }
    }
    else
...
status_t PHY_DP83825_EnableLinkInterrupt(phy_handle_t *handle, phy_interrupt_type_t type, bool enable)
{
    ...
    result = PHY_DP83825_READ(handle, PHY_INTR_CONTROL_STATUS_REG, &regValue);
    if (result == kStatus_Success)
    {
        ...
        result = PHY_DP83825_WRITE(handle, PHY_INTR_CONTROL_STATUS_REG, regValue);
    }
    if (result != kStatus_Success)
    {
        return result;
    ...
    if (enable)
    {
        ...
        if (result == kStatus_Success)
        {
            ...
            result = PHY_DP83825_WRITE(handle, PHY_CONTROL2_REG, regValue);
        }
    }
...
status_t PHY_DP83825_ClearInterrupt(phy_handle_t *handle)
{
    ...
    return PHY_DP83825_READ(handle, PHY_INTR_CONTROL_STATUS_REG, &regValue);
}
